...
def load_dataset(argdict):
    ...
    if argdict['trainset_size']>0:
        ...
        fname = 'data/'+dataset+'/docword_matrix_'+str(argdict['minfreq'])+'_'+str(argdict['trainset_size'])+'traindocs'
    elif argdict['trainset_size']==0:
        print 'using all available train docs'
        fname = 'data/'+dataset+'/docword_matrix_'+str(argdict['minfreq'])

    f = gzip.open(fname+'.pklz','rb')
...
def select_features(mincount=0, dataset='kos'):
    ...
    # saving rest for possible random projection use
