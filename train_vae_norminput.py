...
from vae_2l_norminput import topic_model_2layer
...
if __name__=="__main__":
    ...
    doc_sizes_train = np.squeeze(np.array(csc_matrix.sum(x_train, axis=1)))
    x_test = x_csc[n_total-1-argdict['testset_size']:n_total-1,:] #always same test set
    doc_sizes_test = np.squeeze(np.array(csc_matrix.sum(x_test, axis=1)))
    ...
    while epoch < argdict['max_epochs']:
        ...
        start = time.time()  
        x_train, doc_sizes_train = shuffle(x_train, doc_sizes_train)
        lowerbound, recon_train, KLD, KLD_used = model.iterate(x_train, doc_sizes_train, epoch)
        testlowerbound, recon_test = model.getLowerBound(x_test, doc_sizes_test, epoch)

        if epoch == 1:
            print time.time() - start, 'seconds for first epoch'
        print 'epoch ', epoch, 'lb: ', lowerbound/n_train, 'lb test', testlowerbound/(n_test-n_test%argdict['batch_size']), \
