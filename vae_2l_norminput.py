...
class topic_model_2layer:
    ...
    def createGradientFunctions(self):
        ...
        doc_sizes = T.dvector(name='doc_sizes')

        epoch = T.iscalar('epoch')

        srng = T.shared_randomstreams.RandomStreams()

        H1_lin = th.sparse.dot(self.params['We1'], th.sparse.basic.col_scale(x,1/doc_sizes)) + self.params['be1']
        H1 = relu(H1_lin)
        ...
        self.update = th.function([x, doc_sizes, epoch], [lowerbound, recon_err, KLD, KLD_train, y], updates=updates)
        self.lowerbound  = th.function([x, doc_sizes, epoch], [lowerbound, recon_err], on_unused_input='ignore')
    ...
    def calculate_perplexity(self, doc, selected_features=None, means=None, seen_words=0.5, samples=1):
        ...
        mu, logvar = self.encode(doc_seen/np.sum(doc_seen))
    ...
    def iterate(self, X, doc_sizes, epoch):
        ...
        for i in xrange(0,len(batches)-2):
            ...
            doc_sizes_batch = doc_sizes[batches[i]:batches[i+1]]            
            lowerbound_batch, recon_err_batch, KLD_batch, KLD_train_batch, y = self.update(X_batch.T, doc_sizes_batch, epoch)
    ...
    def getLowerBound(self, data, doc_sizes, epoch):
        ...
        for i in xrange(0,len(batches)-1):
            if batches[i+1]<N:
                ...
                doc_sizes_miniBatch = doc_sizes[batches[i]:batches[i+1]]
                lb_batch, recon_batch = self.lowerbound(miniBatch.T, doc_sizes_miniBatch, epoch)
            else:
